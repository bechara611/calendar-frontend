...
export const useAuthStore =()=>{
    ...
    const startRegister=async({name,email,password})=>{
        ...
        try {
            ...
            console.log(error.response.data.errores.errors)
            dispatch(onLogout(!error.response.data.errores.errors?'INTERNAL ERROR' :error.response.data.errores.errors[0].msg))
            setTimeout(() => {
                dispatch(clearErrorMessage())
            }, 10);
        }
